...
- (void)showBadgeOnItemIndexOne:(int)index {
    [self removeBadgeOnItemIndexOne:index];
    
    //新建小红点
    UIView *bview = [[UIView alloc]init];
    bview.tag = 888+index;
    bview.layer.cornerRadius = 6/2;
...
- (void)hideBadgeOnItemIndexOne:(int)index {
    //移除小红点
    [self removeBadgeOnItemIndexOne:index];
}
- (void)removeBadgeOnItemIndexOne:(int)index {
    for (UIView*subView in self.subviews) {
        if (subView.tag == 888+index) {
            [subView removeFromSuperview];
        }
